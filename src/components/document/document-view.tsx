...
export function DocumentView({ document }: { document: Document }) {
  const [summaryState, summaryFormAction, isSummaryPending] = useActionState(
    getSummary,
    initialSummaryState
  );

  const [answerState, answerFormAction, isAnswerPending] = useActionState(
    getAnswer,
    initialAnswerState
  );
  ...
  useEffect(() => {
    if (
      ...
      summaryState.message !== "Özet başarıyla oluşturuldu."
    ) {
      toast({
        variant: "destructive",
        title: "Hata",
  ...
  useEffect(() => {
    if (
      answerState.message &&
      answerState.message !== "Cevap başarıyla oluşturuldu."
    ) {
      toast({
        variant: "destructive",
        title: "Hata",
        ...
        <TabsContent value="qa" className="mt-4">
          <div className="grid md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <MessageSquare /> AI'a Sor
                </CardTitle>
                <CardDescription>
                  Belge hakkında doğal dilde bir soru sorun.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form action={answerFormAction}>
                  ...
                  <div className="grid w-full gap-2">
                    <Textarea
                      name="question"
                      placeholder="Örn: Bu projenin ana hedefi nedir?"
                      required
                    />
            ...
            <Card className="flex-1">
              <CardHeader>
                ...
                <CardDescription>
                  Önceki sorularınız ve AI cevapları.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {qaHistory.length === 0 ? (
                      ...
                      <div key={index}>
                        ...
                        <div className="space-y-2">
                          <h4 className="text-xs font-semibold text-muted-foreground flex items-center gap-1">
                            <Quote className="h-3 w-3" /> KAYNAKLAR
                          </h4>
